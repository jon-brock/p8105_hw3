...
``` r
#CODE CHUNK COMPLETED (TIME PERMITTING, SEE IF YOU CAN CLEAN IT UP A LITTLE)
...
STEPS (NOT IN ORDER): Variables needed for this: product\_name,
order\_dow, order\_hour\_of\_day GROUP BY ‘order\_dow’ for the days of
week AS.FACTOR ‘order\_dow’ / relevel the variable to be days of week
FILTER == “Pink Lady Apples” and “Coffee Ice Cream” Calculate the mean
value of ‘hour\_of"day’ CREATE knitr::kable table

``` r
instacart %>% 
    select(product_name, order_dow, order_hour_of_day) %>% 
    filter(product_name %in% c("Pink Lady Apples", "Coffee Ice Cream")) %>% 
    mutate(dow = wday(order_dow, label = TRUE, abbr = FALSE)) %>% 
...
```
    ...
    ## Warning: Factor `dow` contains implicit NA, consider using
    ## `forcats::fct_explicit_na`
    
    ## Warning: Factor `dow` contains implicit NA, consider using
    ## `forcats::fct_explicit_na`

| product\_name    |   Sunday |   Monday |  Tuesday | Wednesday | Thursday |   Friday |       NA |
| :--------------- | -------: | -------: | -------: | --------: | -------: | -------: | -------: |
